...
void HexVector::writeToFile(string& file_path) const{
 ofstream myfile;
...
void HexVector::readFromFile(string& file_path){
int index = 0;
string str;
stringstream ss;
ifstream myfile;
...
bool HexVector::checkMove(char c , int rw) const{
...
void HexVector::play(const Cell& move){
...
bool HexVector::checkWin(int curr_row,int curr_col){
	if(getSign() == char(user1)){
...
char HexVector::operator()(int rw, int col) const{
